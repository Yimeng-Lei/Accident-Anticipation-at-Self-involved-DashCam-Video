...
############### Global Parameters ###############
# path
#train_path = '/home/yimeng/self-driving/realistic_data/realistic_dataset/maskrcnn_bundle_2_1024_shuffled/training/'
#test_path = '/home/yimeng/self-driving/realistic_data/realistic_dataset/maskrcnn_bundle_2_1024_shuffled/testing/'
train_path = '/home/yimeng/self-driving/experiments/GCN/keras-deep-graph-learning/examples/data/gcn_output_feature/training/'
test_path = '/home/yimeng/self-driving/experiments/GCN/keras-deep-graph-learning/examples/data/gcn_output_feature/testing/'
#test_path = '/home/yimeng/self-driving/realistic_data/videos/extracted_images/refined_videos/maskrcnn/object_features_pos/old_neg/testing'
demo_path = '/home/yimeng/self-driving/realistic_data/realistic_dataset/maskRCNN_features_1024/testing/'
default_model_path = './model/demo_model'
save_path = './model/model_AdaLEA_GCN/'
#save_path = './model_realistic_2_shuffled_cvpr18/'
video_path = '/home/yimeng/self-driving/realistic_data/realistic_dataset/videos_as_DAD/training/positive'
# batch_number
train_num = 122
#test_num = 50
test_num = 44
...
lam = 3.0 # hyper-param of LEA loss
frame_rate = 20.0 # frame rate
gamma = 5.0 # gamma of AdaLEA
##################################################
...
def build_model():
    ...
    epoch_num = tf.placeholder("float",[None])
    attc = tf.placeholder("float",[None])
    ...
    for i in range(n_frames):
      with tf.variable_scope('model',reuse=tf.AUTO_REUSE):
        ...
        # pos_loss = -tf.multiply(tf.exp(-tf.maximum(0.0, (n_frames-i-1)/20.0) - tf.multiply(lam, epoch_num - 1)),-tf.nn.softmax_cross_entropy_with_logits(logits = pred, labels = y))
        pos_loss = -tf.multiply(tf.exp(-tf.maximum(0.0, (n_frames-i-1) - tf.multiply(frame_rate, attc) - gamma)),-tf.nn.softmax_cross_entropy_with_logits(logits = pred, labels = y))
        # negative example
    ...
    return x,keep,y,optimizer,loss,lstm_variables,soft_pred,all_alphas, epoch_num, attc

def train():
    # build model
    x,keep,y,optimizer,loss,lstm_variables,soft_pred,all_alphas, epoch_num, attc = build_model()
    ...
    for epoch in range(n_epochs):
         ...
         for batch in n_batchs:
             file_name = '%04d' %batch
             batch_data = np.load(train_path+file_name+'.npz')
             batch_xs = batch_data['data']
             batch_ys = batch_data['label']
             _,batch_loss = sess.run([optimizer,loss], feed_dict={x: batch_xs, y: batch_ys, keep: [0.5], epoch_num: [epoch + 1], attc: [tmp_attc]})
         ...
         if (epoch+1) %5 == 0:
            ...
            print("Training")
            tmp_attc = test_all(sess,train_num,train_path,x,keep,y,loss,lstm_variables,soft_pred, epoch_num, attc, tmp_attc)
            print("Testing")
    test_all(sess,test_num,test_path,x,keep,y,loss,lstm_variables,soft_pred, epoch_num, attc, tmp_attc)
    print("Optimization Finished!")
    saver.save(sess, save_path+"final_model")

def test_all(sess,num,path,x,keep,y,loss,lstm_variables,soft_pred, epoch_num, attc, tmp_attc):
    total_loss = 0.0

    for num_batch in range(1,num+1):
         # load test_data
         file_name = '%04d' %num_batch
         test_all_data = np.load(path+file_name+'.npz')
         test_data = test_all_data['data']
         test_labels = test_all_data['label']
         [temp_loss,pred] = sess.run([loss,soft_pred], feed_dict={x: test_data, y: test_labels, keep: [0.0], epoch_num: [1.0], attc: [tmp_attc]})
    ...
    mean_time2acc = evaluation(all_pred,all_labels)
    return mean_time2acc
...
#     ADDED RESULT VAR
    result = np.array([[0] * all_pred.shape[0]] * temp_shape)
    cnt = 0
    AP = 0.0
    for Th_index, Th in enumerate(sorted(all_pred.flatten())):
        ...
        for i in range(len(all_pred)):
            ...
            if len(np.where(all_pred[i]>=Th)[0]) > 0:
                result[Th_index][i] = 1
        if Tp_Fp == 0:
    ...
    new_result = np.array([[0] * all_pred.shape[0]] * len(rep_index))
    for i in range(len(rep_index)-1):
         new_Time[i] = np.max(Time[rep_index[i]:rep_index[i+1]])
         new_Precision[i] = np.max(Precision[rep_index[i]:rep_index[i+1]])
         new_result[i] = result[np.argmax(Time[rep_index[i]:rep_index[i+1]])]

    new_Time[-1] = Time[rep_index[-1]]
    new_Precision[-1] = Precision[rep_index[-1]]
    new_result[-1] = result[rep_index[-1]]
    ...
    #nanor = ~np.isnan(new_Precision)
    #print('new_Time:{}'.format(new_Time))
    #print('type(new_Time):{}'.format(type(new_Time)))
    #print('new_Precision:{}'.format(new_Precision))
    #print('nanor:{}'.format(nanor))
    new_result = new_result[~np.isnan(new_Precision)]
    ...
    sort_result = new_result[np.argsort(new_Recall)]
    print("Recall@80%, Time to accident= " +"{:.4}".format(sort_time[np.argmin(np.abs(sort_recall-0.8))] * 5))
    print('Writing to ./precision_aot/GCN_adaleu')
    np.savez('./precision_aot/GCN_adaleu', Precision = new_Precision, Recall = new_Recall, ToA = new_Time)
    return np.mean(new_Time) * 5
    #print("result:----------------------".format(enumerate(sort_result[np.argmin(np.abs(sort_recall-0.8))])))
#    for tm in enumerate(sort_result[np.argmin(np.abs(sort_recall-0.8))]):
#        print(tm)
...
def vis(model_path):
    ...
    for num_batch in range(1,test_num):
        file_name = '%04d' %num_batch
        all_data = np.load(demo_path+file_name+'.npz')
        data = all_data['data']
